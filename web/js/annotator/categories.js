            ...
            if (annotations.length > 0) {
                for (i = 0, len = annotations.length; i < len; i++) {
                    ...
                    if (annotation.estat == 1 || annotation.permissions.read.length === 0) {
                        isShared = "<img src=\"../css/images/shared-icon.png\" title=\"" + i18n_dict.share + "\" style=\"margin-left:5px\"/>"
                    }
        ...
        Categories.prototype.AnnotationCategory = function (editor, annotation) {
            ...
            if ($('li.annotator-radio').length == 0) { //If the category section not exists
                ...
                for (cat in _categories) {
                    ...
                    radio = radio + '<label for="' + cat + '" style="vertical-align: middle;text-transform:capitalize;"><div class="' + _categories[cat] + ' square" style="display:inline-block;height:15px;width:30px;margin-top:3px;margin-bottom:3px;margin-rigth:5px;vertical-align:middle"></div><span style="margin-left:5px">' + $.i18n._(cat) + '</span></label>';
                    i = i + 1;
